...
export default function Dashboard() {
  ...
  const dueItems = useMemo(() => clients.flatMap((client) => client.vehicles.flatMap((vehicle) => vehicle.dueItems.filter((item) => item.status !== 'green').map((item) => ({ client, vehicle, item })))).sort((a, b) => a.item.daysUntilDue - b.item.daysUntilDue), [clients]);
  const selectedReminder = useMemo(() => dueItems.find(({ client, vehicle, item }) => `${client.id}-${vehicle.id}-${item.kind}` === draftedKey), [dueItems, draftedKey]);
  ...
  const draft = (client: ClientSummary, vehicle: ClientSummary['vehicles'][number], item: DueItem) => {
    const key = `${client.id}-${vehicle.id}-${item.kind}`;
    setDraftedKey(key);
    draftReminder.mutate({ data: { clientName: client.name, model: vehicle.model, plate: vehicle.plate, dueLabel: item.label, dueDate: item.dueDate, daysUntilDue: item.daysUntilDue } }, { onSuccess: (result) => setDraftMessage(result.message) });
  };
  const sendDraft = (client: ClientSummary, vehicle: ClientSummary['vehicles'][number]) => {
    markSent.mutate({ data: { clientId: client.id, vehicleId: vehicle.id, messageText: draftMessage } }, { onSuccess: () => { setDraftMessage(''); setDraftedKey(null); queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); queryClient.invalidateQueries({ queryKey: getListClientsQueryKey() }); } });
  };
    ...
    <div className="mx-auto max-w-[1400px] p-5 sm:p-8 lg:p-11">
      ...
      <div className="mt-10 grid gap-8 xl:grid-cols-[1.1fr_.9fr]">
        <section className="marque-slide-up marque-delay-3 rounded-sm border border-border bg-card p-5 sm:p-7">
          ...
          {dueItems.length === 0 ? <EmptyState title="Nothing pressing" detail="Every registration, policy and service date is in good standing." /> : <div className="divide-y divide-border/70">{dueItems.slice(0, 6).map(({ client, vehicle, item }) => <div key={`${client.id}-${vehicle.id}-${item.kind}`} className="flex flex-wrap items-center gap-3 py-3.5 first:pt-0 last:pb-0" data-testid={`row-attention-${vehicle.id}-${item.kind}`}><div className="min-w-0 flex-1"><Link href={`/clients/${client.id}`} className="text-sm font-bold text-foreground hover:underline" data-testid={`link-attention-client-${client.id}`}>{client.name}</Link><p className="mt-1 text-xs text-muted-foreground">{vehicle.model} <span className="mx-1 text-border">·</span> {item.label}</p></div><div className="text-right"><p className={`font-mono text-xs ${item.daysUntilDue < 0 ? 'text-red-700' : 'text-amber-700'}`}>{relativeDue(item.daysUntilDue)}</p><p className="mt-1 text-[11px] text-muted-foreground">{formatDate(item.dueDate)}</p></div><button type="button" className="rounded-sm border border-border px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-foreground transition hover:border-primary hover:bg-accent" onClick={() => draft(client, vehicle, item)} data-testid={`button-draft-attention-${vehicle.id}-${item.kind}`}>{draftedKey === `${client.id}-${vehicle.id}-${item.kind}` ? (draftReminder.isPending ? 'Writing…' : 'Draft ready') : 'Draft note'}</button></div>)}</div>}
        </section>
